                                                        ...
                                                        cv::InputArray cameraMatrix, cv::InputArray distCoeffs)
{
    ...
    IPPE::PoseSolver Solver;
//	cout << "in front of solve generic" << endl;
    Solver.solveGeneric(objPoints,   imgPoints, cameraMatrix, distCoeffs, Rvec, Tvec, reprojErr1, Rvec2,
                        Tvec2, reprojErr2);
//	cout << "past solve Generic" << endl;
...
//	cout << "calling with type " << normalizedInputPoints.type() << endl;
...
void IPPE::PoseSolver::solveCanonicalForm(cv::InputArray _canonicalObjPoints, cv::InputArray _normalizedInputPoints, cv::InputArray _H,
                                          cv::OutputArray _Ma, cv::OutputArray _Mb)
{
//	cout << "sovleCanonicalForm recieved in type " << _normalizedInputPoints.type() << endl;
...
//	cout << "sent from canonical form with type " << _normalizedInputPoints.type() << endl;
                                   ...
                                   OutputArray _rvec1, OutputArray _tvec1, float& err1, OutputArray _rvec2, OutputArray _tvec2, float& err2)
{
    ...
    //now solve
    cv::Mat Ma, Mb;
	normalizedInputPoints.convertTo(normalizedInputPoints, CV_64FC2); //you changed this, and convert it here
//	cout << "from solve squre, type is " << normalizedInputPoints.type() << endl;
    solveCanonicalForm(objectPoints2D, normalizedInputPoints, H, Ma, Mb);
//	cout << "square past canonical" << endl;
    //sort poses according to reprojection error:
    cv::Mat M1, M2;
    cv::Mat objectPoints3D;
	//cout << "square thing 1" << endl;
    generateSquareObjectCorners3D(squareLength, objectPoints3D);
//	cout << "square thing 1.2";
    sortPosesByReprojError(objectPoints3D, _imagePoints, _cameraMatrix, _distCoeffs, Ma, Mb, M1, M2, err1, err2);
	//cout << "square thing 1.3" << endl;
    //fill outputs
    rot2vec(M1.colRange(0, 3).rowRange(0, 3), _rvec1);
    rot2vec(M2.colRange(0, 3).rowRange(0, 3), _rvec2);
//	cout << "square thing 1.4" << endl;
    M1.colRange(3, 4).rowRange(0, 3).copyTo(_tvec1);
    M2.colRange(3, 4).rowRange(0, 3).copyTo(_tvec2);
//	cout << "done square " << endl;
}
...
//	cout << _normalizedImgPoints.type() << " image points type" << endl;
...
//	cout << "here 1" << endl;
    ...
    float dx, dy;
	//cout << n << " in front of loop " << endl;
	
    for (size_t i = 0; i < n; i++) {
        if (projectedPoints.depth() == CV_32FC1) {
		/*	cout << "type 1" << endl;
			int a = projectedPoints.at<Vec2f>(i)[0];
			cout << "past a" << endl;
			a = imagePoints.at<Vec2f>(i)[0];
			cout << "past i" << endl;
			a = projectedPoints.at<Vec2f>(i)[1];
			cout << "past a2" << endl;
			a = imagePoints.at<Vec2f>(i)[1];
			cout << "past i2" << endl;*/
			...
			/*cout << "type 2" << endl;
			int a = projectedPoints.at<Vec2d>(i)[0];
			cout << "past a" << endl;
			a = imagePoints.at<Vec2f>(i)[0];
			cout << "past i" << endl;
			a = projectedPoints.at<Vec2d>(i)[1];
			cout << "past a2" << endl;
			a = imagePoints.at<Vec2f>(i)[1];
			cout << "past i2" << endl;*/
            dx = projectedPoints.at<Vec2d>(i)[0] - imagePoints.at<Vec2f>(i)[0];
            dy = projectedPoints.at<Vec2d>(i)[1] - imagePoints.at<Vec2f>(i)[1];
        }
