...
void Process::ReadFromPipe(DWORD& dwRead, DWORD& dwWritten, CHAR chBuf[], BOOL bSuccess, HANDLE hParentStdOut, Kalman& kalman) {
	cv::Mat originVector = cv::Mat::zeros(3, 1, CV_64FC1);
	originVector.at<double>(0) = 0;
	originVector.at<double>(1) = 0;
	originVector.at<double>(2) = 1;
	...
	if ((pos = line.find("[")) != std::string::npos && (endPos = line.find("] [", pos + 3)) != std::string::npos) {
		std::string currLine = line.substr(pos + 1, endPos - pos - 1);
		...
		rotation.at<double>(0, 0) = (stod(curr[0]));
		rotation.at<double>(1, 0) = (stod(curr[1]));
		rotation.at<double>(2, 0) = (stod(curr[2].substr(0, curr[2].size() - 1)));
		...
		kalman.rotationMat = R*originVector;
...
//	if ((pos = line.find("]", endPos + 3)) != std::string::npos) {
		...
		line = line.substr(endPos + 3, pos - endPos - 3); //check to make sure this gets rid of the brackets
													   //split by commas
		curr = SplitComma(line);
		for (auto s : curr) {
	//		std::cout << s << " ";
		}
		std::cout << std::endl;
...
void Process::createProcess()
{
	...
	TCHAR tpath[] = TEXT("aruco_test_markermap.exe live C:\\Users\\hithe\\Desktop\\MarkerMapper\\marker_mapper_1.0.12w64\\markerset.yml C:\\Users\\hithe\\Desktop\\MarkerMapper\\marker_mapper_1.0.12w64\\computerOut.yml -s=0.06");// C:\\Users\\hithe\\Desktop\MarkerMapper\\marker_mapper_1.0.12w64\\computerOut.yml - s = 0.06");
...
// Create a child process that uses the previously created pipes for STDIN and STDOUT.
{
	...
	// Create the child process. 
	TCHAR tpath[] = TEXT("aruco_test_markermap.exe live C:\\Users\\hithe\\Desktop\\MarkerMapper\\marker_mapper_1.0.12w64\\school.yml C:\\Users\\hithe\\Desktop\\MarkerMapper\\marker_mapper_1.0.12w64\\computerOut.yml -s 0.06");																																																								// Start the child process. 

	bSuccess = CreateProcess("C:\\Users\\hithe\\Desktop\\MarkerMapper\\marker_mapper_1.0.12w64\\aruco_test_markermap.exe",
		tpath,     // command line 
